...
在集中式训练基础上，用 FedAvg 进行联邦聚合以提升分布式性能。
...
| `--rounds` | 50 | 联邦训练轮数 |
| `--frac` | 0.2 | 每轮参与客户端比例 |
| `--local_epochs` | 1 | 每客户端本地训练轮数 |
| `--lr` | 5e-4 | 本地学习率 |
| `--num_clients` | 0 | 客户端数（0=自动取 L=100） |
| `--split_by` | random | Non-IID 划分策略 |
| `--init_ckpt` | 无 | 热启动检查点路径 |
...
| `models/fedavg_round{NNNN}.pt` | 各轮检查点 |
| `models/fedavg_log.csv` | 逐轮指标记录 |
| `models/fedavg_summary.json` | 汇总报告 |
...
└── models/
    ...
    └── gnn_power_fedavg.pt        # GNN+FL 模型（Step 5 产出）
```

> ⚠️ **重要**：如果缺少这些文件，GNN/GNNFL 算法会自动回退到 EPA（等功率分配），不影响其他算法运行。
...
| **GNN+FL** | 智能 | O(前向传播) | GNN + FedAvg 联邦微调 |
...
消融实验用于学术论文中的 Ablation Study 章节，验证各组件的独立贡献。
...
| 维度 | 对比内容 | 验证目标 |
|------|---------|---------|
| GNN vs No-GNN | GAT vs MLP/线性 | 图结构的必要性 |
| FL vs Centralized | FedAvg vs 集中式 | 联邦学习的增益 |
| Normalization | 全局 vs 样本内 | 归一化策略影响 |
| Data Augmentation | 有无增强 | 数据增强效果 |
| Precoding | MR vs L-MMSE | 预编码方案敏感性 |
...
| **GNN+FL 方案详情** | 算法研究者 | [docs/05. GNN与联邦学习混合功率分配方案.md](docs/05.%20GNN与联邦学习混合功率分配方案.md) |
| **消融实验设计** | 论文作者 | [docs/09. 消融实验设计.md](docs/09.%20消融实验设计.md) |
