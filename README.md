...
Cell-Free 大规模 MIMO 下行链路仿真系统 - 基于 SP-MDGNN 的分布式功率分配研究平台
...
```
传统方案:                    本项目方案:
CPU 收集全部 CSI    →      SP-MDGNN 本地推理 (零通信)
集中计算 ρ 矩阵     →      分布式推理
O(L×K) 同步代价    →      O(1) 无需迭代
```
...
```bash
python train_sp_mdgnn.py --data "../data/gnn_training/*.mat" --epochs 10 --batch_size 64
```
...
| `--model_type` | `sp_mdgnn` | 模型类型：`sp_mdgnn` 或 `glp_lite` |
...
| 文件名 | 说明 |
|--------|------|
| `best_sp_mdgnn_power.pt` | **验证集最优模型**（推荐使用） |
| `final_sp_mdgnn_power.pt` | 最后一轮模型 |
...
CF_downlink_sim/
└── models/
    └── best_sp_mdgnn_power.pt      # SP-MDGNN 推理模型（Step 4 产出）
```
...
| **SP-MDGNN** | 智能 | O(前向传播) | SP-MDGNN 本地推理 |
...
│
├── models/                            # 🧠 模型仓库
│   └── best_sp_mdgnn_power.pt      # 最优 SP-MDGNN 模型
│
└── docs/                              # 📚 详细文档
    ├── 00. 项目概览与导航.md          # 📍 文档入口
