...
for si = 1:length(SNR_dB_range)
    ...
    for modeIdx = 1:length(modes)
        ...
        tmpGain   = cell(nSnapshotsPerSNR, 1);
        tmpD      = cell(nSnapshotsPerSNR, 1);
        tmpSigE   = cell(nSnapshotsPerSNR, 1);
        tmpRhoW   = cell(nSnapshotsPerSNR, 1);
        tmpRhoD   = cell(nSnapshotsPerSNR, 1);
        tmpRhoE   = cell(nSnapshotsPerSNR, 1);
        tmpESRW   = cell(nSnapshotsPerSNR, 1);
        tmpESRD   = cell(nSnapshotsPerSNR, 1);
        tmpESRE   = cell(nSnapshotsPerSNR, 1);
        tmpMeta   = cell(nSnapshotsPerSNR, 1);

        if useParallel
            parfor snap = 1:nSnapshotsPerSNR
                [tmpGain{snap}, tmpD{snap}, tmpSigE{snap}, ...
                 tmpRhoW{snap}, tmpRhoD{snap}, tmpRhoE{snap}, ...
                 tmpESRW{snap}, tmpESRD{snap}, tmpESRE{snap}, ...
                 tmpMeta{snap}] = ...
        ...
        offset = (si-1)*nSnapshotsPerSNR*length(modes) + (modeIdx-1)*nSnapshotsPerSNR;
        for snap = 1:nSnapshotsPerSNR
            idx = offset + snap;
            sqrtGain_cell{idx}  = tmpGain{snap};
            D_cell{idx}         = tmpD{snap};
            sigma_e_cell{idx}   = tmpSigE{snap};
            rho_WMMSE_cell{idx} = tmpRhoW{snap};
            rho_Dist_cell{idx}  = tmpRhoD{snap};
            rho_EPA_cell{idx}   = tmpRhoE{snap};
            ESR_WMMSE_cell{idx} = tmpESRW{snap};
            ESR_Dist_cell{idx}  = tmpESRD{snap};
            ESR_EPA_cell{idx}   = tmpESRE{snap};
            meta_cell{idx}      = tmpMeta{snap};
        end
