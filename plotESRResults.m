...
% 图3: L-MMSE vs Random+L-MMSE vs PSO+L-MMSE (All + DCC)
figure('Visible', 'off');
...
legend('L-MMSE (All)', 'Random+L-MMSE (All)', 'PSO+L-MMSE (All)', ...
    'L-MMSE (DCC)', 'Random+L-MMSE (DCC)', 'PSO+L-MMSE (DCC)');
xlabel('SNR (dB)'); ylabel('Ergodic Sum Rate (ESR)');
title(sprintf('L-MMSE vs Random vs PSO (Avg over %d Scenarios)', numScenarios));
grid on;
if isSaveFig
    saveas(gcf, fullfile(savePath, 'Fig3_LMMSE_vs_Random_vs_PSO.fig'));
    saveas(gcf, fullfile(savePath, 'Fig3_LMMSE_vs_Random_vs_PSO.png'));
end

% 图4: R-MMSE vs Random+R-MMSE vs PSO+R-MMSE (All + DCC)
figure('Visible', 'off');
...
legend('R-MMSE (All)', 'Random+R-MMSE (All)', 'PSO+R-MMSE (All)', ...
    'R-MMSE (DCC)', 'Random+R-MMSE (DCC)', 'PSO+R-MMSE (DCC)');
xlabel('SNR (dB)'); ylabel('Ergodic Sum Rate (ESR)');
title(sprintf('R-MMSE vs Random vs PSO (Avg over %d Scenarios)', numScenarios));
grid on;
if isSaveFig
    saveas(gcf, fullfile(savePath, 'Fig4_RMMSE_vs_Random_vs_PSO.fig'));
    saveas(gcf, fullfile(savePath, 'Fig4_RMMSE_vs_Random_vs_PSO.png'));
end

% 图5: MR vs Random+MR vs PSO+MR (All + DCC)
figure('Visible', 'off');
...
legend('MR (All)', 'Random+MR (All)', 'PSO+MR (All)', ...
    'MR (DCC)', 'Random+MR (DCC)', 'PSO+MR (DCC)');
xlabel('SNR (dB)'); ylabel('Ergodic Sum Rate (ESR)');
title(sprintf('MR vs Random vs PSO (Avg over %d Scenarios)', numScenarios));
grid on;
if isSaveFig
    saveas(gcf, fullfile(savePath, 'Fig5_MR_vs_Random_vs_PSO.fig'));
    saveas(gcf, fullfile(savePath, 'Fig5_MR_vs_Random_vs_PSO.png'));
end
